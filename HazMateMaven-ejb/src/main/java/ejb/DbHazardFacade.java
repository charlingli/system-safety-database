    ...
    public List<Object[]> exportHazards(List<String> searchedHazards) {
        ...
        try {
            querySTR = "SELECT (SELECT COUNT(*) FROM db_control_hazard CtlHaz WHERE CtlHaz.hazardId = Haz.hazardId AND CtlHaz.controlExistingOrProposed = 'E'), "
                        ...
                        + "IFNULL((SELECT GROUP_CONCAT('Yes - ', Fle.fileName ORDER BY Fle.fileId SEPARATOR ',') FROM db_hazard_files HFle, db_files Fle "
                            + "WHERE HFle.fileId = Fle.fileId AND HFle.hazardId = Haz.hazardId GROUP BY HFle.hazardId),'No') "
                     + "FROM db_hazard Haz "
